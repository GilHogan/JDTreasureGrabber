...
严禁用户将其用于大规模、超出个人合理需求的批量抢购行为。

严禁用户利用软件功能进行恶意囤积、扰乱市场秩序或进行商业转卖盈利的行为。

**如果您有将本软件用于上述严禁行为的任何意图，请您立即停止使用本软件，并将其从您的设备上彻底卸载。**
...
请注意，任何自动化、非人工的购买行为，特别是涉及高频或批量的操作，都极有可能违反京东平台（或任何其他电商平台）的用户协议、服务条款及相关规则。

京东平台有权根据其规则对违规账号采取措施，包括但不限于：限制购买、取消订单、冻结或永久封禁账号，甚至追究法律责任。
